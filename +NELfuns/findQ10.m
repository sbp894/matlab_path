...
[f, desc_order_inds]= sort(f, 'descend');
inten= inten(desc_order_inds);
...
hiFreqInd= find(f<frhi, 1); % f is decreasing
if max(inten(hiFreqInd+1:end))<Q10lev
    frlo= min(f); 
elseif inten(temp5(1))< Q10lev %in case not enough data pts were collected
