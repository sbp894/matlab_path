...
function fracSignal= get_trajectory_signal(inSig, fs, freqTrajectory, filtParams)
...
fracSignal= sqrt(2)*filtfilt(d_lp,sig_demod_empirical); % multiply by sqrt(2) because have only considered +ve frequencies,
% have to consider for -ve frequencies too => power should be twice => amplitdue should be sqrt(2) times

fracSignal= abs(fracSignal) / rms(inSig)^2; % normalize by total power