% function [outPower, totPower, Pxx_dB, Freq_xx, Freq_valid_inds]= ...
%     get_freq_trajectory_power(inSig, fs, freqTrajectory, plotPSD, freq_half_window_co_Hz, stim_freq_shift, nw, use_DFT0_DPSS1, freqMin)
% Compute power in a signal along a frequency trajectory. numel(inSig) has
% to be equal to numel(freqTrajectory). 
% Options to use both multitaper PSD (pmtm) or DFT.
...
elseif isempty(stim_freq_shift)
    stim_freq_shift= geomean(freqTrajectory);
end
...
if use_DFT0_DPSS1==0
    dft_sidedness= 1;
    [Adb, F_dft]= plot_dft(sig_Plot, fs, 'sided', dft_sidedness, 'dc', true, 'plot', plotPSD==1);
    ...
    if plotPSD
        hold on;
        plot([stim_freq_shift-freq_half_window_co_Hz stim_freq_shift+freq_half_window_co_Hz], [max(Adb)+2 max(Adb)+2], 'color', get_color('g'), 'LineWidth', 4);
    end
    ...
    totPower= 2*sum((db2mag(Adb(dft_legalFreqInds))./dft_mask(dft_legalFreqInds)).^2)*numel(sig_demod_empirical)/nfft_true;
    %     Pxx_dB= (db2mag(Adb)./dft_mask).^2*numel(sig_demod_empirical)/nfft_true;
    Pxx_dB= Adb;
    Freq_xx= F_dft;
    Freq_valid_inds= dft_validFreqInds;
else
    ...
    [Pxx_dB, Freq_xx]= plot_dpss_psd(sig_Plot, fs, 'nw', nw, 'title', '', 'norm', true, 'plot', plotPSD==1, 'yrange', 60);
    ...
    Freq_valid_inds= dpss_validFreqInds;
end